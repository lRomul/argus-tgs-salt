...
class UNetProbResNet(nn.Module):
    def __init__(self, encoder_depth, num_classes, num_filters=32, dropout_2d=0.2,
        ...
        self.prob_fc = nn.Linear(num_filters * 8, 1)
    ...
    def forward(self, x):
        ...
        prob = self.prob_avg_pool(center)
        prob = prob.view(prob.size(0), -1)
        prob = self.prob_fc(prob)
        prob = self.sigmoid(prob)
        prob = prob.view(-1)

        x = self.final_conv(F.dropout2d(dec0, p=self.dropout_2d))
        x = self.sigmoid(x)

...
class SeResnextFPNProb50(nn.Module):
    def __init__(self, num_classes, num_channels=3,
                 dropout_2d=0.2, last_filters=32, final='softmax',
                 skip_dropout=None, fpn_layers=None):
        super().__init__()
        self.num_classes = num_classes
        self.num_channels = num_channels
        if dropout_2d:
            self.dropout_2d = nn.Dropout2d(p=dropout_2d)
        else:
            self.dropout_2d = None

        assert final in ['softmax', 'sigmoid', 'logits'], \
            "Incorrect output type. Should be 'softmax', 'sigmoid' or 'logits'"
        ...
        # Decoders
        self.decoders = nn.ModuleList([DecoderBlock(n_filters[0] + 64, n_filters[0],
                                                    is_deconv, True)])
        for i in range(1, len(n_filters) - 1):
            scale = True if i != 1 else False
            self.decoders.append(DecoderBlock(2 * n_filters[i], n_filters[i - 1],
                                              is_deconv, scale))
        self.decoders.append(DecoderBlock(n_filters[-1], n_filters[-2],
                                          is_deconv, True))

        if skip_dropout and dropout_2d:
        ...
        if fpn_layers is None:
            fpn_layers = [128 for _ in range(len(n_filters))]

        assert len(n_filters) == len(fpn_layers), \
        ...
        for i in range(n_blocks):
            if i != 1:
                self.fpn_blocks.append(FPNBlock(n_filters[i],
                                                fpn_layers[i], c_scale))
            else:
                self.fpn_blocks.append(FPNBlock(n_filters[i] - 64,
                                                fpn_layers[i], c_scale))
            if i != 1:
                c_scale = c_scale * 2
        ...
        self.finalconv2 = nn.Conv2d(fpn_filters, last_filters, 3,
                                    padding=1)
        self.finalrelu2 = nonlinearity(inplace=True)
        self.finalconv3 = nn.Conv2d(last_filters, num_classes, 1,
                                    padding=0)

        # Prob branch
        self.pool = nn.MaxPool2d(2, 2)
        self.prob_avg_pool = nn.AdaptiveAvgPool2d(1)
        self.prob_fc = nn.Linear(n_filters[-1], 1)
        self.sigmoid = nn.Sigmoid()
